...
export async function scanOnce(env: Env): Promise<PickRow[]> {
  ...
  for (const sport_key of sports) {
    ...
    for (const ev of events) {
      ...
      const commence = ev.commence_time as string;
      ...
      for (const bk of ev.bookmakers || []) {
        ...
        if (!m) continue;
        const names = (m.outcomes || []).map((o: any) => o.name);
        const prices = (m.outcomes || []).map((o: any) => Number(o.price));
        if (!prices.length || prices.some((x) => !Number.isFinite(x))) continue;
      ...
      const best: { name: string; odds: number; book: string | null }[] =
        allNames.map((n) => ({ name: n, odds: 0, book: null }));
      for (const bk of ev.bookmakers || []) {
        const m = (bk.markets || []).find((x: any) => x.key === "h2h");
        if (!m) continue;
        for (const out of m.outcomes || []) {
          ...
          if (idx >= 0 && price > (best[idx].odds || 0)) {
            best[idx] = {
              name: out.name,
              odds: price,
...
export async function settlePending(env: Env) {
  // evenimentele încă "upcoming" a căror oră a trecut
